...
public class BookCatalog {
    ...
    public static void main(String[] args) {
        ...
        books.stream().filter(s -> s.getAuthor().startsWith("A")).forEach(System.out::println);
        System.out.println("---------------");
        books.stream().filter(s-> s.match(s)).forEach(System.out::println);
        //identiska, ena med en separat metod & andra med endast lambda
        books.stream().filter(s-> s.getAuthor().startsWith("A") || s.getAuthor().startsWith("B") || s.getAuthor().startsWith("C"));
        System.out.println("---------------");
        System.out.println("Enter the letter you don't want the books to contain");
        String choice = sc.nextLine();
        System.out.println("---------------");
        Long booksRemoved = books.stream().filter(s-> s.getTitle().contains(choice)).count();
        Long newBooks = 30 - booksRemoved;
        System.out.println("Nr of books removed: "+newBooks);
        
        //same as above but shorter
        Long nrOfBooksremoved = books.stream().filter(s-> !s.getTitle().contains(choice)).count();
